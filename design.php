...
$getShirtDB = "select id, collection_theme_id, name, description, shirt_collar_id, shirt_cuff_id, shirt_pocket_id, shirt_yoke_id, shirt_placket_id, shirt_button_id, shirt_back_id, shirt_bottom_id, shirt_fit_id, price, is_short_sleeve from collection_shirt WHERE id = ".$_SESSION['shirt_id'];
$getShirt = createQuery($getShirtDB, $conn);

$row = mysql_fetch_row($getShirt);

$shirt_id = $row[0];
$collection_theme_id = $row[1];
$name = $row[2];
$description = $row[3];
$shirt_collar_id = $row[4];
$shirt_cuff_id = $row[5];
$shirt_pocket_id = $row[6];
$shirt_yoke_id = $row[7];
$shirt_placket_id = $row[8];
$shirt_button_id = $row[9];
$shirt_back_id = $row[10];
$shirt_bottom_id = $row[11];
$shirt_fit_id = $row[12];
$price = $row[13];
$is_short_sleeve = $row[14];
...
<div style="width:950px; overflow:hidden; text-align:left; margin-left:auto; margin-right:auto">
    <div id="shirt" style="position: absolute; margin-top: 125px; width: 250px; color:#FFF; margin-left:auto; margin-right:auto">
        <div id="small">
			...
			<?php
				$main_picture = "SELECT image, big_image FROM collection_gallery WHERE collection_shirt_id = ".$_SESSION['shirt_id']." AND main <> 'Y' ORDER BY main ASC"; 
				$main_picture = createQuery($main_picture, $conn);
				$count_check = 0;
				while ($info = mysql_fetch_array($main_picture)) {
					$image = $info['image'];
					$big_image = $info['big_image'];
					if($count_check == 0){
						echo "<a href=\"$big_image\" class=\"jqzoom\" rel='gal1' title='$description' >
								<img src=\"$image\" class=\"another\"\">
							</a>";
											
						echo "<span id=\"small_images\" style=\"width:250px; height:56px; float:left; overflow:hidden;\"><p style=\"margin-top:0px; \">";
					}
					echo "<a ";
					if($count_check == 0)
						echo "class=\"zoomThumbActive\" ";
						
					echo "href='javascript:void(0);' rel=\"{gallery: 'gal1', smallimage: '$image',largeimage: '$big_image'}\">
							<img src='$image' class=\"small\" width=\"40\" style=\"padding:5px 5px 5px 5px; float:left\"></a>";
					$count_check++;
...
<img src="images/monogram/rightpocket.png" style="display:none" />
                        ...
                        <tr height="30">
                            ...
                            <td><input class="monogram_text text ui-widget-content ui-corner-all" type="text" placeholder="e.g R.O.N" maxlength="5" /></td>
                        </tr>
