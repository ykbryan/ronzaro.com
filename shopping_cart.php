                    ...
                    <div style="width:100%; margin:10px 0px 10px 0px" id="cart_content">
                        ...
                        if($_SESSION['cart']){
							...
							foreach($_SESSION['cart'] as $key=>$value){
								...
								$imageDB = "select image from collection_gallery WHERE main = 'Y' AND collection_shirt_id = " . $shirt->collection_shirt_id;
								$image = createQuery($imageDB, $conn);
								
								$row = mysql_fetch_row($image);
								
								echo "<div style=\"float:left; width:120px;\" align=\"center\">
										<img src=\"$row[0]\" width=\"100\" style=\"padding-top:10px; \">
										<span style=\"font-size:10px; color:#666\"><br />x <a class=\"remove\" id=\"$key\">Remove this shirt</a></span>
									</div>";
								
								$nameDB = "select name, price from collection_shirt WHERE id = " . $shirt->collection_shirt_id;
								$name = createQuery($nameDB, $conn);
								...
								echo "<div style=\"float:right; width:470px;\">
										<div style=\"padding:10px 10px 10px 10px; margin-left:10px; border-bottom:#333 1px solid\">
											<strong>$row[0]</strong>
											<span style=\"float:right\">USD $row[1] per piece</span>
										</div>";
								...
								//check quantity
								$quantityDB = "SELECT quantity FROM collection_shirt WHERE id = $shirt->collection_shirt_id"; 
								$quantity = createQuery($quantityDB, $conn);
								
								$row = mysql_fetch_row($quantity);
								$total_quantity = $row[0];
								
								$quantityDB = "SELECT count(a.id) FROM account_shirt a INNER JOIN payment p ON a.payment_id = p.id WHERE a.collection_shirt_id = $shirt->collection_shirt_id AND p.date_paid != \"0000-00-00 00:00:00\""; 
								$quantity = createQuery($quantityDB, $conn);
								
								$row = mysql_fetch_row($quantity);
								$sold_quantity = $row[0];
								
								$quantity = intval($total_quantity)-intval($sold_quantity);
                            ...
                            <ul style="border-top:1px #999 dashed; list-style: square; font-size:10px; padding-top:10px; margin-left:0px; color:#999; width:100%" id="shirt_summary">
                                ...
                                if($_SESSION['cart']){
                                    foreach($_SESSION['cart'] as $key=>$value){
										...
										echo " class=\"item\" style=\"margin-left:15px\"><span id=\"highlight_$key\" class=\"highlight\" style=\"position:absolute; width:150px; height:12px; background-color:#AAA; opacity:0.5; \"></span><strong>$row[0]</strong> <span id=\"quantity_$key\" style=\"color:#FFF\">";
										if($shirt->quantity >1)
										...
										echo "<input type=\"hidden\" name=\"item_name_$x\" id=\"item_name_$x\" value=\"$row[0]\">
												<input type=\"hidden\" name=\"item_number_$x\" id=\"item_number_$x\" value=\"$shirt->collection_shirt_id\">
												<input type=\"hidden\" name=\"amount_$x\" id=\"amount_$x\" value=\"$row[1]\">
